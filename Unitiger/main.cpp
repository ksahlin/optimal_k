...
int initialize_de_bruijn_graph(Graph& graph, string reads, size_t k, size_t abundance)
{
    ...
    else 
    {
        ...
        if (filecount > 1) 
        {
            ...
            graph = Graph::create(b, (char const *)"-kmer-size %d -abundance %d -bloom cache -debloom original -verbose 1", k, abundance);
        } 
        else 
        {
            graph = Graph::create ((char const *)"-in %s -kmer-size %d -abundance %d -bloom cache -debloom original -verbose 1", reads.c_str(), k, abundance);
        }
  }
