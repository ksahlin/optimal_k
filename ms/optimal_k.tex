...
Therefore, we also get an estimate of $X := |V(\DB)|$ as $\hat{X} = \hat{p}_{k,a}|\K_k| = \hat{p}_{k,a} n(r-k+1)$. By the theory in Sec.~\ref{sec:sampling-accuracy}, we immediately get how many samples we need in order to bound the relative error within a certain confidence interval.
...
The estimate of $Y := |\ST|$ is then $\hat{Y} = \hat{q}_{k,a}|\K_k| = \hat{q}_{k,a}n(r-k+1)$. By the theory in Sec.~\ref{sec:sampling-accuracy}, we immediately get how many samples we need in order to bound the relative error within a certain confidence interval.
\alex{say we can implement this for all abundaces.}
...
It is enough to first estimate the average number of unary (i.e. internal) nodes of of a unitig, and then obtain the average string length by summing $k + 1$.\footnote{This assumes, in order to simplify the presentation, that the dBG has no isolated nodes, which are unitigs with $0$ internal nodes but spell strings of length $k$. However, isolated nodes can be easily accounted for as separate case in all the formulas.} Since above we have obtained an estimate for the number of unitigs, the idea here is to estimate also the number of unary nodes, and then obtain the estimate for the average number of nodes by diving the latter by the former.

Let $\UN$ denote the set of unary nodes of $\DB$. As above, we can write
...
Consider the following procedure which produces a multiset $W$ containing the unitigs of $\DB$: for all $k$-mers $x \in \K_k$, if $x$ is a node of $\DB$, output all the unitigs containing $x$. Each unitig $w := (v_1,v_2,\dots,v_t)$ of $\DB$ appears $\alpha(w) := \frac{1}{t}\sum_{i = 1}^{t}\alpha(v_i)$ times in $W$. Therefore, we can equivalently express the E-size of the set $\U$ as 
...
\end{equation}
Notice that in the relation (\ref{eqn:sample-size}) above, both $p$ and $\hat{p}$ are not known at the start of the sampling, when the value of $n$ needs to be chosen. \alex{In our case, we choose ...  }
