...
\SetKwBlock{extend}{{\sf extend}$(x,y,A_1,A_2)$}{end}
...
\begin{algorithm}[h]
\caption{A $k$-mer $x$ and an out-neighbor $y$ of $x$. The output is the The estimate $\hat{p}_{k,a}$ needed for the number of $k$-mers in the de Bruijn graph $\DB$, for all $a \in [A_1,A_2]$. The input is also a multiset $\{x_1,\dots,x_m\}$ of $k$-mers from $\K_k$.\label{alg:sampling-for-esize}}
...
\tcp{\small We extract the min and max abundance in $start$}
$min_a = \min(start)$\;
$max_a = \max(start)$\;
...
\If{$start \neq \emptyset$}
{
    ...
    \ForEach{$b \in \{\mathtt{A}, \mathtt{C}, \mathtt{G}, \mathtt{T}\}$}
    {
    	$y = x[2..k]b$\;
    	$\abu(y) = \RLCSA.count(y)$\;
		\tcp{\small We try extending the unitig starting with $x,y$ as long as there is an abundance for which it is unary}
    	$extension\_length = {\sf extend}(x,y,min\_a, \min(\abu(y),max\_a))$\;
	
    	\ForEach{$a \in start$}
    	{
			\If{$min\_a \leq a \leq \min(\abu(y),max\_a)$}
			{
				$\abu[a].append(\abu(y))$\;
				$length[a].append(extension\_length[a])$
			}
...
\Return $(length[a],\abu[a])$, for all $a \in [A_1,A_2]$.
\end{algorithm}
